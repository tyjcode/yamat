...
#################################################################
        self.handlerEvents = CollisionHandlerEvent()
        self.handlerEvents.addInPattern("into-%in")
        self.handlerEvents.addOutPattern("outof-%in")

        traverser = CollisionTraverser() 
        self.cTrav = traverser # cTrav は、特別なオブジェクト。このトラバーサーは、 フレームごとに自動的に実行される
        self.cTrav.addCollider(pandaC, self.handlerEvents)
        
        self.accept("into-pandaActor2", self.intocollision)
        self.accept("outof-pandaActor2", self.outofcollision)

    def intocollision(self, entry):
        print("into")
        print(entry)
        #self.pandaActor.stop()
        self.pandaPace.pause()
        pandaPos = self.pandaActor.getPos()
        i1 = self.pandaActor.posInterval(2,Point3(pandaPos.x, pandaPos.y-3, pandaPos.z + 8))
        i2 = self.pandaActor.posInterval(2,Point3(pandaPos.x, pandaPos.y-20, pandaPos.z))
        self.pandaJump = Sequence(i1,i2)
        self.pandaPJ = Parallel(self.pandaPace, self.pandaJump)
        self.pandaJump.start()
    
        

        
    def outofcollision(self, entry):
        print("outof")
        print(entry)
        # self.pandaJump.pause()
